...
const ReviewRow = ({ handleRemoveReview, review }) => {
	...
	useEffect(() => {
		fetch(`https://server-side-roan.vercel.app/service/${service_id}`)
			.then((res) => res.json())
	...
	const handleUpdate = (event) => {
		...
		fetch(`https://server-side-roan.vercel.app/reviews/${reviewId}`, {
			method: 'PATCH',
			headers: {
	...
	return (
		<tr>
			...
			<td>
				<textarea
					className='w-full rounded-none p-3 font-medium resize-none'
					disabled
